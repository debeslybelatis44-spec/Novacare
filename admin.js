...
function displayPatientAdmin(patient) {
    ...
    if (patient.privilegeGrantedDate) {
        ...
        if (hoursDiff >= 24) {
            ...
            // Supprimer le compte crédit associé
            if (state.creditAccounts[patient.id]) {
                delete state.creditAccounts[patient.id];
            }
            expired = true;
        }
    }
...
// Sauvegarde des notes du patient
...
function savePrivilege() {
    ...
    // Nettoyer les anciens privilèges
    patient.vip = false;
    patient.sponsored = false;
    patient.discountPercentage = 0;
    patient.hasCreditPrivilege = false;
    ...
    // Supprimer l'ancien compte crédit s'il existe
    if (state.creditAccounts[patientId]) {
        delete state.creditAccounts[patientId];
    }
    ...
    if (privilegeType === 'vip') {
        ...
        state.transactions.forEach(t => {
            if (t.patientId === patientId && t.status === 'unpaid' && !t.isCredit) {
                // Sauvegarder le montant original si pas déjà fait
                if (!t.originalAmount) {
                    t.originalAmount = t.amount;
                }
                t.amount = t.originalAmount * (1 - discountPercentage / 100);
            }
        });
        ...
        // Aucun privilège : on ne fait que nettoyer (déjà fait)
        alert("Privilèges retirés du patient");
    }
    
    saveStateToLocalStorage();
    searchAdminPatient(); // Rafraîchit l'affichage
}
